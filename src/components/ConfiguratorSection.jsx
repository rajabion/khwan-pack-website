...
const RIM_PARAMS = { radius: 0.8, tube: 0.05, radialSegments: 16, tubularSegments: 100 };
const PLASTIC_PARAMS = { topRadius: 0.8, bottomRadius: 0.6, height: 2, segments: 64 };
...
function OriginalCupMesh({ type, logoUrl }) {
    ...
    const fallbackLogo = 'data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7';
    ...
    return (
        <group dispose={null}>
            {type === 'paper' && (
                <group>
                    <mesh position={[0, 1, 0]} castShadow receiveShadow material={paperMaterial}>
                    ...
                    <mesh position={[0, 2, 0]} rotation={[Math.PI / 2, 0, 0]} material={paperMaterial}>
                        <torusGeometry args={[RIM_PARAMS.radius, RIM_PARAMS.tube, RIM_PARAMS.radialSegments, RIM_PARAMS.tubularSegments]} />
                    </mesh>
                </group>
            )}
            ...
            {type === 'plastic' && (
                <group>
                    ...
                    <mesh position={[0, 1, 0]}>
                        ...
                        <meshBasicMaterial color="white" transparent opacity={0.0} depthWrite={false} side={THREE.DoubleSide} />
                        {logoUrl && (
                            <Decal position={[0, 0, 0.75]} rotation={[0, 0, 0]} scale={decalScale}>
                ...
                <div className="order-2 lg:order-1 flex flex-col gap-8">
                    <div className="glass-card p-12 rounded-[48px] border border-white/40 shadow-2xl text-start">
                        ...
                        <div className="space-y-10">
                            <div>
                                <label className="block text-sm font-bold text-slate-400 mb-4 uppercase tracking-widest">{t('configurator.cup_type')}</label>
                                <div className="flex bg-slate-100 p-2 rounded-2xl gap-2">
                                    <button onClick={() => setCupType('paper')} className={`flex-1 py-4 rounded-xl font-bold transition-all ${cupType === 'paper' ? 'bg-white text-blue-700 shadow-md' : 'text-slate-500 hover:text-slate-800'}`}>{t('configurator.paper')}</button>
                                    <button onClick={() => setCupType('plastic')} className={`flex-1 py-4 rounded-xl font-bold transition-all ${cupType === 'plastic' ? 'bg-white text-blue-700 shadow-md' : 'text-slate-500 hover:text-slate-800'}`}>{t('configurator.plastic')}</button>
                                </div>
                            </div>

                            <div>
                                <label className="block text-sm font-bold text-slate-400 mb-4 uppercase tracking-widest">{t('configurator.upload_label')}</label>
                                <label htmlFor="upload" className="flex flex-col items-center justify-center w-full h-48 border-2 border-dashed border-slate-300 rounded-[32px] cursor-pointer hover:border-blue-500 hover:bg-blue-50/50 transition-all group">
                                    <Upload className="w-10 h-10 text-slate-400 group-hover:text-blue-600 mb-4" />
                                    <span className="text-slate-700 font-bold text-lg">{t('configurator.upload_btn')}</span>
                ...
                <div className="order-1 lg:order-2 h-[600px] lg:h-[800px] w-full bg-white/40 backdrop-blur-2xl rounded-[60px] overflow-hidden relative border border-white/60 shadow-inner">
                    <Canvas shadows dpr={[1, 2]} camera={{ position: [0, 0, 5], fov: 40 }}>
                        <Suspense fallback={null}>
                            ...
                            <OrbitControls makeDefault enableZoom={true} minDistance={3} maxDistance={12} />
                        </Suspense>
                    </Canvas>
                    ...
                    <div className="absolute top-8 start-8 p-5 glass rounded-3xl flex items-center gap-4 border border-white/50">
                        ...
                        <div className="text-start">
                            <p className="text-sm font-bold text-slate-900 leading-none mb-1">{t('configurator.preview_label')}</p>
                            <p className="text-[10px] text-slate-500 font-bold uppercase tracking-wider">High-End Rendering</p>
                        </div>
                    </div>
                </div>
            </div>
