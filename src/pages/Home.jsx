...
const Section = ({ title, subtitle, children, align = "center", className = "" }) => (
    <section className={`min-h-[60vh] flex flex-col items-center justify-center p-8 relative z-10 ${className}`}>
        <motion.div
        ...
        >
            ...
            {subtitle && <p className="text-xl text-blue-700 font-bold mb-8 tracking-wide">{subtitle}</p>}
            ...
            <Section title={t('services.title')} subtitle={t('services.subtitle')}>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8 text-center mt-8">
                    <div className="p-6 rounded-3xl bg-slate-50 border border-slate-100 flex flex-col items-center">
                        <div className="w-16 h-16 bg-blue-100 text-blue-600 rounded-2xl flex items-center justify-center mb-4 font-extrabold">01</div>
                        <h4 className="font-bold text-xl mb-3">{t('services.s1_title')}</h4>
                        <p className="text-sm text-slate-500 font-medium">{t('services.s1_desc')}</p>
                    </div>
                    <div className="p-6 rounded-3xl bg-slate-50 border border-slate-100 flex flex-col items-center">
                        <div className="w-16 h-16 bg-red-100 text-red-600 rounded-2xl flex items-center justify-center mb-4 font-extrabold">02</div>
                        <h4 className="font-bold text-xl mb-3">{t('services.s2_title')}</h4>
                        <p className="text-sm text-slate-500 font-medium">{t('services.s2_desc')}</p>
                    </div>
                    <div className="p-6 rounded-3xl bg-slate-50 border border-slate-100 flex flex-col items-center">
                        <div className="w-16 h-16 bg-yellow-100 text-yellow-600 rounded-2xl flex items-center justify-center mb-4 font-extrabold">03</div>
                        <h4 className="font-bold text-xl mb-3">{t('services.s3_title')}</h4>
                        <p className="text-sm text-slate-500 font-medium">{t('services.s3_desc')}</p>
                    </div>
                </div>
            </Section>

            {/* 5. Portfolio - Success Partners */}
            <Section id="portfolio" title={t('nav.portfolio')} subtitle="SUCCESS PARTNERS">
                <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4 md:gap-6 mt-4">
                    {["Glaze", "Black Rose", "First Exit", "Repose", "Joe's Cafe", "Phi Boutique", "Veloce"].map(client => (
                        <motion.div
                            key={client}
